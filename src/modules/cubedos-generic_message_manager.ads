...
--
-- This package defines the core CubedOS messaging system. It is instantiated once for each
-- domain in a distributed CubedOS system. It handles message passing within the domain and
-- routes messages into other domains. Every domain must have a unique ID as decided upon by
...
is
   ...
   -- An endpoint associated with exactly one module used to send and receive messages. It also
   -- stores metadata about the module used to prove aspects about message safety. Modules should
   -- keep their instance of this object hidden.
   ...
   -- Sends the given message to the specified module and domain. This procedure asserts that
   -- the destination module may receive the given message type and that the module does exist
   -- in the destination domain. It is the safest method of sending a message and should be
   -- prefered. The procedure returns immediately and moves the message's payload making it
   -- inaccessible after the call.
   --
   -- This procedure generally shouldn't be called directly and will be used in auto-generated
   -- API files.
