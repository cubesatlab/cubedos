...
package body Networking_Client.Messages is
   use Message_Manager;
   use type Ada.Real_Time.Time;
   use type Networking_Server.API.Status_Type;
   ...
   procedure Handle_Ping_Reply(Message : in Message_Record) with
     Pre => Networking_Server.API.Is_Ping_Reply(Message)
   is
      Status           : Networking_Server.API.Status_Type;
      Decode_Status    : Message_Status_Type;
      Round_Trip_Time  : Ada.Real_Time.Time_Span;
   begin
      Networking_Server.API.Ping_Reply_Decode(Message, Status, Decode_Status);
      ...
      elsif Status /= Networking_Server.API.Success then
   ...
   begin
      if Networking_Server.API.Is_Ping_Reply(Message) then
