...
class SemanticAnalyzer(
  ...
  override def visitStruct_body(ctx: MercParser.Struct_bodyContext): TypeRep.Rep = {
    visitChildren(ctx).asInstanceOf[ComponentRep]
  }

  override def visitType_def(ctx: MercParser.Type_defContext): TypeRep.Rep = {
  ...
  override def visitDeclaration(ctx: MercParser.DeclarationContext): TypeRep.Rep = {
   //TODO
    TypeRep.NoTypeRep
  }

  def visitType_specifier2(ctx: MercParser.Type_specifierContext): String = {
  ...
  def visitRange_constraint2(ctx: MercParser.Range_constraintContext): String = {
  ...
  def enterConstant_def2(ctx: MercParser.Constant_defContext): Any = {
  ...
  def enterType_def2(ctx: MercParser.Type_defContext): Any = {
    if (ctx.getChild(0).getText == "typedef" && ctx.declaration().getChild(0).getText != "opaque") {
      if (ctx.range_constraint().getChild(2).getText.contains(MercParser.CONSTANT)) {
        val sid = "null"
  ...
  override def exitRange_constraint(ctx: MercParser.Range_constraintContext): Unit = {
